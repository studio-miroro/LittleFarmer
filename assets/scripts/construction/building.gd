...
@onready var buildings:Node = get_node("/root/"+main+ "/Buildings")
@onready var tilemap:Node2D = get_node("/root/"+main+ "/Tilemap")
@onready var grid:Node2D = get_node("/root/"+main+ "/Buildings/Grid")
@onready var collision:Node2D = get_node("/root/"+main+ "/Buildings/Grid/GridCollision")
...
func build(tile_mouse_pos:Vector2i, building_id:int, building_node:PackedScene, node_shadow:CompressedTexture2D) -> void:
	#var shadow_group:CanvasGroup = get_node("/root/"+main+ "/Shadow")

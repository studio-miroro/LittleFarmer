...
@onready var letter_node:PackedScene = load("res://assets/nodes/UI/popup/mails/letter.tscn")
...
@onready var Button_getItems:Button = $Panel/HBoxContainer/ContentScroll/VBoxContainer/Items/VBoxContainer/ButtonContainer/GetItems
...
func check_all_keys(id:int, dictionary:Dictionary):
		...
		if letters[_index].has("items"):
			if letters[_index]["items"] != {}:
				...
				if !letters[_index]["items"].has("collected"):
					Button_getItems.visible = true
				else:
					Button_getItems.visible = false
...
func create_letters(dictionary:Dictionary, node:PackedScene, parent:VBoxContainer):
	for i in dictionary:
...
func letter_create_items(id:int, amount:int, parent:GridContainer, node:PackedScene):
...
func letter_delete_items(parent:GridContainer, node:PackedScene):
	for child in parent.get_children():
		parent.remove_child(child)
		child.queue_free()
